...
import Backend.Manejadores.ManejadorDeErrores;
...
public class BinaryTree <E> {
    ...
    public void insert(Object dato){
        if(this.find(raiz, dato) == null){
            if(!isInsertionWithIDRepeated(dato)){
    ...
    private boolean isInsertionWithIDRepeated(Object dato){
        if(find(raiz.getLeftChild(), dato) != null){
            insert(raiz.getLeftChild(), 1, dato);
            
            return true;
        }else if(find(raiz.getRightChild(), dato) != null){
            insert(raiz.getRightChild(), 1, dato);
        ...
        if (raiz == null) {
            ...
            if(nodoAnterior != null){//a mi parecer nunca llegaría a ser null, pero por si xD
                ...
                if (herramientas.esMayor(nodoAnterior.getContent(), nuevoNodo.getContent())){///no hay problemas con un null pointer porque está el if para cuando la raiz es null... xD y solo en ese caso el elemento insertado no tendría un anterior...                   
                    ...
                    System.out.println("Se ha insertado el dato "+ dato);
                }/*else{//este else se vuelve inncesario por la revisión de existencia que se realiza previamente... aunque podría hace la revisión aquí, y así ahorrarme ese paso que agrega carga al programa...
                    System.out.println("Ya se encuentra almacendo un dato igual");
                    this.manejadorErrorres.addError("NO se puede insertar cartas repetidas");
    ...
    public Node delete(Object dato){
        Node nodoAEliminar = find(raiz, dato);
